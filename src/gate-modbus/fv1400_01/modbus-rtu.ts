...
interface IModbusRTU {
     ...
     * @param {TransmissionSpeedType} transmissionSpeed
     */
    setTransmissionSpeed: (transmissionSpeed: TransmissionSpeedType) => void
    /**
     ...
     * @param {AlwaysWriteMultipleType} alwaysWriteMultiple
     */
    setAlwaysWriteMultiple: (alwaysWriteMultiple: AlwaysWriteMultipleType) => void
    /**
...
class ModbusRTU implements IModbusRTU {
     ...
     */
    setTransmissionSpeed(transmissionSpeed: TransmissionSpeedType): void {
        this.raw.set(PropertyType.TransmissionSpeed, transmissionSpeed);
    }
    /**
     * Ustawia sprawdzanie parzystości
     * @param {ParityType} parity
     ...
     */
    setRefreshPeriod(refreshPeriod: number): void {
        this.raw.set(PropertyType.RefreshInterval, refreshPeriod);
    }
    /**
     * Ustawia adres obsługiwanego  rejestru
     * @param {number} registerAddress
     ...
     */
    setAlwaysWriteMultiple(alwaysWriteMultiple: AlwaysWriteMultipleType): void {
        this.raw.set(PropertyType.AlwaysWriteMultiple, alwaysWriteMultiple);
    }
    /**
     * Ustawia liczbę dyskretnych wejść / wyjść podlegających operacji odczytu / zapisu
     * @param {number} inputOutputCount
...
class ModbusRTURemote implements IModbusRTU {
    ...
    setTransmissionSpeed(transmissionSpeed: TransmissionSpeedType): void {
        const cmd: string | null = rawExecutionBuilderFactory(this.objectName)
            ...
            .addParameter(transmissionSpeed)
            .build();
        this.gate.runScript(cmd!);
    }
    /**
    ...
    setAlwaysWriteMultiple(alwaysWriteMultiple: AlwaysWriteMultipleType): void {
        const cmd: string | null = rawExecutionBuilderFactory(this.objectName)
            ...
            .addParameter(alwaysWriteMultiple)
            .build();
        this.gate.runScript(cmd!);
    }
    /**
...
export { ModbusRTU, ModbusRTURaw, ModbusRTURemote, TransmissionSpeedType, ParityType, StopBitsType, RegisterTypeType, AlwaysWriteMultipleType, DataTypeType, DataWidthType, EndiannessType, InitialValueAccessType }
