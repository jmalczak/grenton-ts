// Created from: src/interfaces/module_infibity_thermostat_radiator_valve_ff.xml, object name="ZWAVE_CONFIG"
...
interface IZwaveConfig {
    ...
    /** Pobiera wartość danego rejestru (parametru) konfiguracyjnego:\n0 - DataType UINT -  przychodząca odpowiedź jest bez znaku (wartość tylko dodatnia)\n1 - DataType INT -  przychodząca odpowiedź jest ze znakiem (dodatnia bądź ujemna) */
    get: (register: number, dataType?: number) => void
...
class ZwaveConfig implements IZwaveConfig {
    ...
    get(register: number, dataType: number = 0): void { this.raw.execute(MethodType.Get, register, dataType); }
...
class ZwaveConfigRemote implements IZwaveConfig {
    ...
    get(register: number, dataType: number = 0): void {
        const cmd = rawExecutionBuilderFactory(this.objectName).execute().addParameter(MethodType.Get).addParameter(register).addParameter(dataType).build();
        this.gate.runScript(cmd!);
    }
    setDefault(register: number): void {
